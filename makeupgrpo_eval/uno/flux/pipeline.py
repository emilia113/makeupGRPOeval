...
class UNOPipeline:
    ...
    def forward(
        ...
        inp_cond = prepare_multi_ip(    # prepare_multi_ip会在x和ref_imgs在batch维度不一致时，将ref_imgs在bsz维度上进行复制
        ...
        # 批量后处理 -> List[PIL.Image]
        x = x.clamp(-1, 1)                     # 仍假设输出在 [-1, 1]
        x = rearrange(x, "b c h w -> b h w c") # [B, H, W, C]
        imgs_uint8 = (127.5 * (x + 1.0)).to(torch.uint8).cpu().numpy()

        pil_images = [Image.fromarray(img) for img in imgs_uint8]
        return pil_images
